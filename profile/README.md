# Always-On Security

Always-On Security is a containerized, multi-layer security monitoring and enforcement platform designed for HPC and air-gapped container environments.

## Repositories

### Always-On-Security

The primary repository containing the complete platform implementation, including the security monitoring pipeline, risk engine, dashboard, deployment configuration, and supporting services.

### HPE-CPP-Resources

Supporting documentation, architecture references, setup guides, and additional resources for contributors working on the main project.

## Getting Started

For installation, deployment, and usage instructions, refer to the documentation in the **Always-On-Security** repository.
